...
// SEARCH FUNCTIONS implementing debouncing and auto-suggestion------
...
// interval of 300ms for search result
...
function SearchProducts() {
  ...
  const filterProducts = Products.filter(product => product.title.toLowerCase().includes(query) || product.category.toLowerCase().includes(query));
...
//2) create Auto-suggestion dropdown to show 5 matching products (With debounce logic applied) 
document.getElementById('search').addEventListener('input', () => {
  const query = document.getElementById('search').value.toLowerCase();
  if (query === "") {
  ...
  const filterProducts = Products.filter(product => product.title.toLowerCase().includes(query) || product.category.toLowerCase().includes(query));
  renderAutoSuggestions(filterProducts);
});

function renderAutoSuggestions(products) {  
  const autoSuggestions = document.getElementById("auto-suggestions");
  autoSuggestions.innerHTML = ""; 
  
  products.slice(0, 5).forEach((product) => {
    const suggestion = document.createElement("div");
    suggestion.innerText = product.title;
    suggestion.onclick = () => {
      window.location.href = `productDetail.html?id=${product.id}`;
    };
  ...
  if (products.length === 0) {
    autoSuggestions.innerHTML = `<p>No Auto-suggestion found</p>`;
  }
...
// function for logout() to redirect back to  login page
